...
async function startServer() {
  ...
  app.post("/api/send-invite", async (req, res) => {
    ...
    try {
      ...
      // For now, we'll try to use a test account or log it if not configured
      ...
      if (!smtpHost || !smtpUser || !smtpPass) {
        ...
        return res.json({ success: true, message: "Link generated (email not sent due to missing SMTP config)", link });
      }
